...
const NotificationCard = ({data, index, notificationState}) => {
    ...
    let {seen, type, reply, createdAt, comment, replied_on_comment, user, user: {personal_info: {username, fullname, profile_img}}, blog: {_id, blog_id, title}, _id: notification_id} = data;
    let [isReplying, setReplying] = useState(false);
    let {userAuth: {username: author_username, profile_img: auhtor_profile_img, access_token}} = useContext(UserContext);
    ...
    const handleDelete = (comment_id, type, target) => {
        ...
        axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/delete-comments", 
        { _id: comment_id}, {
            headers: {
        ...
        <div className="flex gap-5 mb-3">
            ...
            <div className="w-full">
                <h1 className="font-medium text-dark-grey text-xl">
                    ...
                    <Link to={`/user/${username}`} className="max-1 text-black underline">@{username}</Link>
        ...
        } 
        {
            ...
            <div className="ml-20 p-5 bg-grey mt-5 rounded-md">
                <div className="flex gap-3 mb-3">
                    <img src={auhtor_profile_img} className="w-8 h-8 rounded-full"/>
                    <div>
                        <h1 className="font-medium text-xl text-dark-grey">
                            <Link to={`/user/${author_username}`} className="max-1 text-black underline">@{author_username}</Link>
                             <span className="font-normal">
                                replied to
                             </span>
                            <Link to={`/user/${username}`} className="max-1 text-black underline">@{username}</Link>
                ...
                <p className="ml-14 text-xl font-gelasio my-2">
                    {reply.comment}
                </p>
